...
- 동적 웹페이지 : 사용자가 웹서버에 요청을 해서 값이 변경되는 것
    ...
    <img src="./image/web007.png" width="600">
...
- 윈도우 설정은 일반 사용자용, 제어판은 개발자용
    ...
    <img src="./image/web008.png" width="600">
...
- EntitiyFramework DB연동 방식 : ORM(Object-Relational Mapping) 방식
    - 제일 최근의 DB연동 기법
    - Spring Boot JPA, myBatis와 동일
...
2. NuGet 패키지 관리자
    ...
    - `MySql.EntityFrameworkCore` 8.0.14 (Db First 시 반드시 필요함!!)
    - Pomelo.EntityFrameworkCore.MySql 8.0.3
3. appsettings.json에 DB연결 문자열 추가
...
5. 아래 내용 입력
    ...
    -a----        2025-05-29   오후 4:57         1133 Day07Study.sln 
...
1. 뉴스 게시글 수정
    ...
    - Create.cshtml과 Edit.cshtml에서 필요없는 `입력값 필드`(Writer, PostDate, ReadCount) 삭제
...
4. HTML 에디터 추가
    ...
    - Create.cshtml, Edit.cshtml `<input aso-for="Description">` -> `<textarea>`로 변경
    - Site.js 마지막에 thrumbowyg 초기화 함수 추가작성
    - 뷰어 라이브러리 : Westwind.AspNetCore.Markdown NuGet 패키지 라이브러리
    - Detail.schtml, Delete.cshtml에 라이브러리 using Westwind.AspNetCore.Markdow 라이브러리 import
...
4. 정적페이지 DB연동
    ...
    - NuGet 패키지 관리자 콘솔
        - Add-Migration
        - Update-Database
    - HomeController의 About()에 메서드에서 DB연동
...
5. 회원가입 / 로그인
    ...
    - Models.ApplicationDbContext 에서 기반클래스를 Identity
