...
namespace MyPortfolioWebApp.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            // userManager나 signInManager에 널 값이 들어오면 안됨
        ...
        public async Task<IActionResult> Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await userManager.CreateAsync((CustomUser)user, model.Password);   // 자동으로 DB에 저장

                if (result.Succeeded)
                {
                    // 위의 저장한 유저로 로그인
                    // isPersistent는 로그인 상태 유지를 계속하는지 여부. false하면 20~30분 동안 사용안하면 로그아웃
                    await signInManager.SignInAsync((CustomUser)user, isPersistent: false);
