...
namespace MyPortfolioWebApp.Controllers
{
    public class NewsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int page = 1, string search = "")
        {
            ...
            // 쿼리로 처리가능. DB 저장프로시저로도 처리 가능
            //var news = _context.News.FromSql($@"SELECT Id, Writer, Title, Description, PostDate, ReadCount
            //                                    FROM News")
            ...
            // 최종단계
            // 페이지 개수
            //var totalCount = _context.News.FromSql($@"select Id from News where Title like '%{search}%'").Count(); // 전체 게시글 수 (현재 오류 발생 밑 줄 코드 사용!)
            var totalCount = _context.News.Where(n => EF.Functions.Like(n.Title, $"%{search}%")).Count();   
            var countList = 10; // 한 페이지에 기본 뉴스 개수 10개
        ...
        public async Task<IActionResult> Create([Bind("Id,Title,Description")] News news, IFormFile? UploadFile)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (UploadFile != null && UploadFile.Length > 0) {
                    ...
                    Directory.CreateDirectory(uploadFolder);    // 폴더가 없으면 생성
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,Title,Description")] News news, IFormFile? NewFile)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // UPDATE News SET ...
                    //_context.Update(news);    // 방식1 - ID가 같은 새글을 업데이트하면 수정
                    // COMMIT
                    await _context.SaveChangesAsync();
